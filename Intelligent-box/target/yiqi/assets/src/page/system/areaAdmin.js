define(function(require, exports, module) {
    ...
    var Admin = Backbone.Model.extend({
        ...
        destroy: function(options) {
            ...
            var id  = this.get('id')
            $.ajax({
                url: CONTEXT_PATH + '/web/system/deleteArea.do',
                type: 'post',
                dataType: 'json',
                context: this,
                data: {
        ...
        save: function(attrs, options) {
            Admin.__super__.save.call(this, attrs, _.extend({}, this.syncOptions, options));
        },
        toggleStatus: function() {
            var data = this.pick('tid');
            ...
            confirm(confirmText, function (){
                that.save(null, {
                    url: CONTEXT_PATH + '/web/system/updateAreaStatus.do',
                    context: that,
                    data: data,
    ...
    var adminCreateModalRender = template($('#tmpl-adminCreateModal').html());
    var AdminCreateModal = FormModal.extend({
        template: adminCreateModalRender,
        events: {
            'change #inputAccount' : 'doCheck',
        ...
        initAreaPicker: function(areaIds) {
            this.areaPicker = new AreaPicker({
                el: this.$('.area-picker'),
            });
        }
    });
    var adminEditModalRender = template($('#tmpl-adminEditModal').html());
    var AdminEditModal = FormModal.extend({
        template: adminEditModalRender,
        submit: function(event) {
            if (this.$form.valid()) {
                var $target = $(event.target);
                var data = this.$form.serializeObject();
                ...
                this.model.save(null, {
                    url: CONTEXT_PATH + '/web/system/editareaAdmin.do',
                    context: this,
                    data: data,
                    success: function(model, resp, options) {
                        resp = _.extend({
                        ...
                        if (resp.success) {
                            // var attrs = _.pick(data, ['name', 'mobile', 'provincename', 'cityname', 'areaname']);
    ...
    var ItemView = Backbone.View.extend({
        ...
        doEdit: function() {
            Backbone.trigger('edit:admin', this.model, this);
        },
    ...
    var DataTable = Backbone.View.extend({
        ...
        bulkEnable: function() {
            ...
            if (selection.length === 0) {
                ...
                this.bulkStatus(selection, Admin.STATE_ENABLE);
            }
        },
        bulkDisable: function() {
            ...
            if (selection.length === 0) {
                ...
                this.bulkStatus(selection, Admin.STATE_DISABLE);
            }
        },
    ...
    function run() {
        ...
        _.extend(list, {
            ...
            url: CONTEXT_PATH + '/web/system/listarea.do',
            parse: function(resp) {
                var parsed = _.extend({
                    success: false,
                    model: []
        ...
        var modal;
        $('[data-do="create:admin"]').on('click', function() {
            modal instanceof FormModal && modal.remove();
            modal = new AdminCreateModal({
                model: new Admin(),
                collection: list
            });
        ...
        Backbone.on('edit:admin', function(model) {
            modal instanceof FormModal && modal.remove();
            modal = new AdminEditModal({
                model: model,
                collection: list
            });
            modal.render().$el.appendTo(document.body);
