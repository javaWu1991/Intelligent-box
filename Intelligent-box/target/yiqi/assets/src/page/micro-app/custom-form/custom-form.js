define(function (require, exports, module) {
    ...
    var iconImages = ['bx_1','bxiao','h_bx','h_cc','h_ly','h_qj','h_sp','h_wc','img_bmxz','img_byj',
            'img_cg','img_dh','img_lxsq','img_wp','waichu','ywlxd','yz_1'];
    ...
    var SettingView = Backbone.View.extend({
        ...
        changeSelects: function (e) {
            ...
            if (action === 'del') {
                ...
                else { elem.parent().after(selectRender({ index: length + 1 })); }
            }
        ...
        toDataJSON: function (sequence) {
            if (!this.setting.valid()) {
                ...
                var model = _.extend({}, emptySettingJson, {
                    ...
                    sequence: sequence,
                    controlId: this.type,
                    exp: this.data.placeholder,
                    value: this.data.selects.length > 0 ? { options: this.data.selects } : '',
                    jsonData: this.data.selects.length > 0 ? { options: this.data.selects } : '',
                });
                return model;
            }
        }
    ...
    var parseJsonToSetting = function (jsonData) {
        return _.extend({}, emptySettingData, {
            ...
            placeholder: jsonData.exp,
            required: jsonData.isRequired === 'true',
            type: jsonData.controlId,
            unit: '',
            selects: jsonData.jsonData ? jsonData.jsonData.options : []
        });
    };
    var initPage = function (widgets, data) {
        ...
        widgetPanel.on('addWidget', function (type) {
            ...
            if (settingPanel.currentWidget && settingPanel.currentWidget.type === 'TableField'
                ...
                settingPanel.currentWidget.append(widget);
            }
            else if (settingPanel.currentWidget && settingPanel.currentWidget.parentContainer.type === 'TableField'
                && widget.type !== 'TableField') {
                settingPanel.currentWidget.parentContainer.append(widget);
            }
            else {
