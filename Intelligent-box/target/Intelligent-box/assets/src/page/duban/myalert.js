...
(function($){  
    $.malert=function (option) {  
        ...
        var _w = option.width || "1.5";  
        var _h = option.height || "1";  
        var _text = option.text;  
        var _type = option.type;  
        var _buttons = option.buttons;  
        //弹框类型  
        if(_type=='tip'){  
            var _ceng_bg = '<div style="width:60%;height:50%;position:fixed;top:0;left:0;background:#000;opacity:0" class="tck"></div>';  
            var _ceng_con = '<div style="width:' + _w + _unit + ';height:' + _h + _unit + ';border-radius:' + _radius + 'px;position:fixed;top:50%;left:50%;background:#222;opacity:0.9">';  
            _ceng_con += '<p class="' + _name + '_p" style="color:#fff;font-size:.16rem;text-align:center;padding:0 .3rem"><span>' + _text + '</span></p>';  
            _ceng_con += '</div>';  
            ...
            $('.' + _name + '_p').css({"margin-top": 0.2 + "rem"});  
            $('.' + _name + '_p').parent('div').css({"height": (_ceng_con_p_h ) + "rem","width": (_ceng_con_p_w + 0.6*2) + "rem","margin-left":-(_ceng_con_p_w + 0.6*2)/2+"rem","margin-top":-(_ceng_con_p_h + 0.6)/2+"rem"});  
            setTimeout(function(){  
                $('.' + _name + '_p').parent('div').prev('div').remove();  
                $('.' + _name + '_p').parent('div').remove();  
            },1200);  
        }else if(_type=='tip02'){  
            var _ceng_bg = '<div style="width:60%;height:50%;position:fixed;top:0;left:0;background:#000;opacity:' + _bg_opacity + '"></div>';  
            var _ceng_con = '<div style="width:' + _w + _unit + ';height:' + _h + _unit + ';border-radius:' + _radius + 'px;position:fixed;top:50%;left:50%;background:#fff">';  
            _ceng_con += '<div style="width:60%;height:40%;position:relative">';  
            _ceng_con += '<div style="width:60%;height:' + (_h - 1.35) + 'rem;padding-top:1px">';  
            _ceng_con += '<p class="' + _name + '_p" style="color:#666;font-size:.4rem;text-align:center;padding:0 .5rem">' + _text + '</p>';  
            _ceng_con += '</div>';  
            _ceng_con += '<div style="width:60%;height:0.2rem;position:absolute;bottom:.35rem;left:0">';  
            if(_buttons){  
                if(_buttons.length<2){  
                    _ceng_con += '<a href="javascript:void(0)" class="' + _name + '_cancle_btn" style="display:block;height:0.2rem;margin:0 .35rem;border-radius:4px;text-align:center;line-height:0.2rem;background:#73d13a;color:#fff;font-size:.38rem">'+_buttons[0]+'</a>';  
                }else{  
            ...
            var _ceng_con = '<div style="width:' + _w + _unit + ';height:' + _h + _unit + ';border-radius:' + _radius + 'px;position:fixed;top:50%;left:50%;background:#fff">';  
            _ceng_con += '<div style="width:60%;height:40%;position:relative">';  
            _ceng_con += '<div style="width:60%;height:' + (_h - 1) + 'rem;padding-top:1px">';  
            _ceng_con += '<p class="' + _name + '_p" style="color:#666;font-size:.4rem;text-align:center;padding:0 .5rem">' + _text + '</p>';  
            _ceng_con += '</div>';  
            _ceng_con += '<div style="width:60%;height:0.2rem;position:absolute;bottom:0;left:0;border-top:1px solid #e6e6e6">';  
            if(_buttons){  
                if(_buttons.length<2){  
